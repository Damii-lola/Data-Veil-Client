...
function drawBg() {
  ...
  // Scrolling grid
  ctx.strokeStyle = 'rgba(0,220,255,0.028)';
  ctx.lineWidth = 1;
  const gs = 60;
  ...
  particles.forEach((p, i) => {
    ...
    if (Math.random() < 0.008) p.char = '0123456789ABCDEF'[Math.floor(Math.random()*16)];
  });
...
// Idle rotation
const idleMsgs = ['SYSTEM READY — AWAITING INPUT','ALL SYSTEMS NOMINAL','CIPHER ENGINE ONLINE','16-STEP ALGORITHM LOADED'];
let idleIdx = 0;
setInterval(() => {
...
// ── CHAR COUNT ──
inputText.addEventListener('input', () => { charCountEl.textContent = inputText.value.length; });
...
function renderStats(data) {
  ...
  const uniqueHexChars = new Set(hex.toUpperCase()).size;
  const entropy = Math.round(uniqueHexChars / 16 * 100);
  const items = [
    ...
    block.innerHTML = `
      ...
      <div class="step-details">
        <table class="steps-table">
          <tr><td>STEP 2</td><td>Split → <span class="val-highlight">First3:</span> ${escHtml(s.first3)} &nbsp;|&nbsp; <span class="val-highlight">Mid2:</span> ${escHtml(s.next2)} &nbsp;|&nbsp; <span class="val-highlight">Last4:</span> ${escHtml(s.last4)}</td></tr>
          <tr><td>STEP 3</td><td>a=${s.step3.a}, b=${s.step3.b}, c=${s.step3.c} → (a+b)=${s.step3.sum_ab}, ×c=${s.step3.prod}, sum=${s.step3.sum_abc}<br>mod1=${s.step3.mod1}, mod2=${s.step3.mod2} → <span class="val-highlight">Carry=${s.step3.carry}</span></td></tr>
          <tr><td>STEP 4</td><td>${s.step3.carry} × ${s.step4.midVal} = <span class="val-highlight">${s.step4.step4res}</span></td></tr>
          <tr><td>STEP 5</td><td>Set1=[${s.step5.set1.join(', ')}] &nbsp; Set2=[${s.step5.set2.join(', ')}]<br>Diffs=[${s.step5.diffs.join(', ')}] → <span class="val-highlight">ScrambleSum=${s.step5.scrambleSum}</span></td></tr>
          <tr><td>STEP 6</td><td>${s.step5.scrambleSum} ÷ ${s.step4.step4res} = <span class="val-highlight">${s.step6.quotient}</span> (integer)</td></tr>
          <tr><td>STEP 7</td><td>base5(${s.step6.quotient}) = '${escHtml(s.step7.base5Str)}' → dec ${s.step7.base5AsDecimal} × ${s.first3} = <span class="val-highlight">${s.step7.product}</span></td></tr>
          <tr><td>STEP 8</td><td>digitSum='${s.step8.sumBase5}' → divisor1=<span class="val-highlight">${s.step8.divisor1}</span> &nbsp;|&nbsp; last4Sum=${s.step8.last4Sum} → divisor2=${s.step8.divisor2}<br>chainNums=[${s.step8.chainNumbers.join(', ')}] → sum=${s.step8.chainSum} → <span class="val-highlight">chainFinal=${s.step8.chainFinal}</span></td></tr>
          <tr><td>STEP 9</td><td>weave(${s.step8.divisor1}, ${s.step8.chainFinal}, ${s.step8.div2_8}) = <span class="val-highlight">'${trunc(s.step9.weaved,30)}'</span></td></tr>
          <tr><td>STEP 10</td><td>→ base5:'${trunc(s.step10.base5_2,24)}' → base7:'${trunc(s.step10.base7,24)}' → <span class="val-highlight">hex:'${escHtml(s.step10.hex)}'</span></td></tr>
          <tr><td>STEP 11</td><td>sig=${escHtml(s.step11.sig)} → wrapped: <span class="val-highlight">'${trunc(s.step11.wrapped,30)}'</span></td></tr>
          <tr><td>STEP 12</td><td>first4='${escHtml(s.step12.first4)}' ↔ last4='${escHtml(s.step12.last4part)}' → <span class="val-highlight">'${trunc(s.step12.swapped,28)}'</span></td></tr>
          <tr><td>STEP 13</td><td>hex → decimal: ${trunc(s.step13.base10,40)}</td></tr>
          <tr><td>STEP 14</td><td>→ base4: ${trunc(s.step14.base4,40)}</td></tr>
          <tr><td>STEP 15</td><td>→ base9: ${trunc(s.step15.base9,40)}</td></tr>
          <tr><td>STEP 16</td><td><span class="val-final">${escHtml(s.step16.finalHex)}</span></td></tr>
        </table>
      </div>`;
...
inputText.addEventListener('keydown', e => { if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') encrypt(); });
